...
///
/// The folder is inside [`install_dir`], so for a vendor runtime each version
/// has its own copy: `launch.exe` names a program relative to that directory,
/// and a component shared across versions would sit outside it. A component
/// is always pinned, never a vendor download of its own.
pub fn plan_component(
    ...
    if component.runtime.source == RuntimeSource::Vendor {
        return Err(missing(descriptor, "a pinned download for each part of it"));
    }
    let base = install_dir(descriptor, host, runtime_root, runtime_version)?;
    let dir = component_dir(&base, name);
    plan_runtime(descriptor, &component.runtime, dir, present, None)
}
...
mod tests {
    ...
    fn a_component_is_planned_into_its_own_folder_with_its_own_pin() {
        let plan =
            plan_component(&with_jre(), "win32-x64", "C:\\rt", "jre", &NOTHING, None).unwrap();
        assert_eq!(
            plan,
            Plan::Direct {
    ...
    fn a_component_already_on_disk_is_not_fetched_again() {
        ...
        assert!(matches!(
            plan_component(&with_jre(), "win32-x64", "/rt", "jre", &present, None).unwrap(),
    ...
    fn an_undeclared_component_is_refused() {
        assert!(plan_component(&with_jre(), "win32-x64", "/rt", "nope", &NOTHING, None).is_err());
    }
