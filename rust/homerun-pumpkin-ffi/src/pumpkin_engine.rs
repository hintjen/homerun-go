...
use pumpkin::{PumpkinServer, init_logger, reset_server_state, stop_server};
...
impl Engine for PumpkinEngine {
    fn run(
        ...
        let outcome = runtime.block_on(async {
            ...
            config.advanced.networking.java.address.set_port(request.java_port);
            ...
            MAX_PLAYERS.store(config.advanced.networking.java.max_players, Ordering::Relaxed);

            let vanilla_data = VanillaData::load();
            init_logger(&config.advanced);

            let server =
                match PumpkinServer::new(config.basic, config.advanced, vanilla_data).await {
                    Ok(server) => server,
                    Err(e) => {
