...
///
/// The config text comes from `homerun-core`, which is what the desktop and
/// Android will render from too. The same gateway is on the other end of all
/// three, so divergence is a bug by definition — and the way to guarantee that
/// is one generator, not three that agree today.
@MainActor
final class WireProxy {
    ...
    /// Render the wireproxy configuration, in `homerun-core`.
    ///
    /// This used to be built here, as a list of strings mirroring the desktop's
    /// generator by hand — and Android had a third copy of the same list. The
    /// doc comment on all three said "divergence is a bug by definition",
    /// which is true and is exactly why none of them should exist: the gateway
    /// is one thing, so its config is one thing.
    ///
    /// What the core encodes that a hand-written copy keeps getting wrong:
    /// every `ListenPort` is **fixed by what the gateway DNATs to**, whatever
    /// port the server actually bound. Only `Target` follows the local one. A
    /// host that "corrects" a listen port to match produces a config that
    /// loads, connects, and is unreachable.
    ///
    /// TCP only here. Desktop and Android also render UDP sections for Bedrock
    ...
    /// Bedrock is rejected before it reaches here. The core is told `java`,
    /// and refuses an exposure it does not know rather than guessing.
    static func render(link: Link, minecraftPort: Int) throws -> String {
        // The core uses the API's field names, not Swift's. Absent address and
        // allowed-IPs mean the legacy gateway, and the core supplies the same
        // fixed pair this used to hardcode — so they are omitted rather than
        // defaulted here, keeping one place that knows the fallback.
        var payload: [String: Any] = [
            "client_privkey": link.clientPrivateKey,
            "gateway_pubkey": link.gatewayPublicKey,
            "link_address": link.endpoint,
        ]
        if let address = link.address { payload["address"] = address }
        if let allowedIps = link.allowedIps { payload["allowed_ips"] = allowedIps }

        return try Core.renderTunnel(link: payload, port: minecraftPort, exposure: "java")
    }
    ...
    ///
    /// That is true as of the `ParseConfigString` patch. Before it, the Go
    /// wrapper wrote the config to a temp file because wireproxy's parser only
